...
import Modelo.tercero.Cuota_Constante_modelo;
 ...
 */
public class Cuota_Constante extends javax.swing.JFrame {

    DecimalFormat formato = new DecimalFormat("#");
    DefaultTableModel dtm = new DefaultTableModel();
    
    public Cuota_Constante() {
        initComponents();
        this.setLocationRelativeTo(null);
        
        String[] titulo = new String[]{"#","Valor de la Cuota","Interes","Saldo Deuda"};
        dtm.setColumnIdentifiers(titulo);
        tabla.setModel(dtm);
        
    }

    public void agregarDatosTabla(double prestamo, int cuota, double anualidad){
        double interesCalculado = 0;
        double restaPrestamo = prestamo;
        
        for(int i = 1; i <= cuota; i++){
            
            interesCalculado = Math.round(restaPrestamo*0.01);
            restaPrestamo = Math.round(restaPrestamo+interesCalculado-anualidad);    
            
            if (restaPrestamo <= 0) {restaPrestamo = 0;}
            
            dtm.addRow(new Object[]{
                i,
                formato.format(anualidad),
                formato.format(interesCalculado),
                formato.format(restaPrestamo),
            });
        }       
    }
    ...
    private void initComponents() {
        ...
        Menu1.setBorder(javax.swing.BorderFactory.createTitledBorder("Cuota Constante"));
        ...
        getContentPane().add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 20, 790, 30));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        Cuota_Constante_modelo sm = new Cuota_Constante_modelo(prestamo, interes, cuota);
        
        limpiarTabla();
        
        double anualidad = sm.anualidad();       
        lbTotal.setText(""+formato.format(anualidad));
        ...
        agregarDatosTabla(prestamo, cuota, anualidad);
